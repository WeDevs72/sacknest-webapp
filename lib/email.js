...
/**
 * Send free prompts PDF email
 * @param {Object} params - Email parameters
 * @param {string} params.customerEmail - Customer's email address
 * @returns {Promise<Object>} - Nodemailer response
 */
export async function sendFreePromptsEmail({ customerEmail }) {
  try {
    if (!customerEmail) {
    ...
    const defaultSupabaseUrl = `${supabaseUrl}/storage/v1/object/public/assets/free_prompts.pdf`
    
    // User can still override with FREE_PDF_URL in .env if they host it elsewhere
    const downloadUrl = process.env.FREE_PDF_URL || defaultSupabaseUrl
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f5f5f5;">
  <table role="presentation" style="width: 100%; border-collapse: collapse;">
    <tr>
      <td align="center" style="padding: 40px 0;">
        <table role="presentation" style="width: 600px; max-width: 100%; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="background: linear-gradient(135deg, #facc15 0%, #f59e0b 100%); padding: 40px 30px; text-align: center;">
              <h1 style="margin: 0; color: #000000; font-size: 32px; font-weight: 900; text-transform: uppercase;">
                ✨ SackNest
              </h1>
              <p style="margin: 10px 0 0; color: #000000; font-size: 16px; font-weight: 600;">
                Premium AI Prompt Library
          ...
          <tr>
            <td style="padding: 40px 30px; text-align: center;">
              <h2 style="margin: 0 0 10px; color: #000000; font-size: 28px; font-weight: 800;">
                Here are your Free Prompts! 🎉
              </h2>
              <p style="margin: 0 0 30px; color: #6b7280; font-size: 16px; font-weight: 500;">
                Thanks for joining the SackNest community. You can download your free premium prompts below.
              </p>
              
              <a href="${downloadUrl}" style="display: inline-block; background: linear-gradient(135deg, #000000 0%, #333333 100%); color: #facc15; text-decoration: none; padding: 18px 48px; border-radius: 12px; font-weight: 800; font-size: 18px; text-transform: uppercase; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.3);">
                📥 Download PDF
              </a>
            </td>
          </tr>
          ...
          <tr>
            <td style="background-color: #000000; padding: 30px; text-align: center; border-top: 4px solid #facc15;">
              <p style="margin: 0 0 20px; color: #9ca3af; font-size: 14px; font-weight: 600;">
                Ready for more? Explore our Premium Packs at SackNest!
              </p>
    ...
    const mailOptions = {
      from: process.env.SMTP_FROM_EMAIL || '"SackNest" <' + process.env.SMTP_USER + '>',
      to: customerEmail,
      subject: '🎉 Your Free AI Prompts from SackNest',
      html: htmlContent
    }

    const info = await transporter.sendMail(mailOptions)
    console.log('Free prompts email sent successfully:', info.messageId)
    return { success: true, messageId: info.messageId }

  } catch (error) {
    console.error('Error sending free prompts email:', error)
    return { error: error.message }
  }
}
